...
function animationOnScroll() {
    for(let i = 0; i < animations.length; i++){
        ...
        if (animElHeight > window.innerHeight) animElPoint = window.innerHeight - window.innerHeight / animStart;
        if (window.scrollY > animElOffset - animElPoint && window.scrollY < animElOffset + animElHeight) animEl.classList.add('show');
        else if (!animEl.classList.contains('animation--no-hide')) animEl.classList.remove('show');
    }
}
function offset(el) {
